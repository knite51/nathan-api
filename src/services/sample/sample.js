...
class SampleService extends RootService {
  ...
  async create_record(request, next) {
    try {
      ...
      const err = this.process_failed_response(
        `[SampleService] created_record: ${e.message}`,
        500
      );
      next(err);
    }
  ...
  async read_record_by_id(request, next) {
    try {
      ...
      const result = await this.sample_controller.read_records({
        id,
      ...
      const err = this.process_failed_response(
        `[SampleService] update_record_by_id: ${e.message}`,
        500
      );
      return next(err);
    }
  ...
  async read_records_by_filter(request, next) {
    try {
      ...
      const err = this.process_failed_response(
        `[SampleService] read_records_by_filter: ${e.message}`,
        500
      );
      next(err);
    }
  ...
  async read_records_by_wildcard(request, next) {
    try {
      ...
      const err = this.process_failed_response(
        `[SampleService] read_records_by_wildcard: ${e.message}`,
        500
      );
      next(err);
    }
  ...
  async update_record_by_id(request, next) {
    try {
      ...
      const result = await this.sample_controller.update_records(
        { id },
        { ...data }
      );
  ...
  async update_records(request, next) {
    try {
      ...
      const err = this.process_failed_response(
        `[SampleService] update_records: ${e.message}`,
        500
      );
      next(err);
    }
  ...
  async delete_record_by_id(request, next) {
    try {
      ...
      const err = this.process_failed_response(
        `[SampleService] delete_record_by_id: ${e.message}`,
        500
      );
      next(err);
    }
  ...
  async delete_records(request, next) {
    try {
      ...
      const result = await this.sample_controller.delete_records({
        ...seek_conditions,
      });
      ...
      const err = this.process_failed_response(
        `[SampleService] delete_records: ${e.message}`,
        500
      );
      next(err);
    }
